...
class CreateDiscussionsTable extends Migration
{
    ...
    public function up()
    {
        DB::unprepared('CREATE TABLE discussions (
            `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
            `topic` LONGTEXT NULL,
            `user_id` BIGINT NOT NULL,
            `basin_id` INT NOT NULL,
            `image_key_id` VARCHAR(200) NULL,
            `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
            `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            PRIMARY KEY (`id`),
            INDEX `idx_discussions_user_id` (`user_id` ASC),
            INDEX `idx_discussions_basin_id` (`basin_id` ASC),
            INDEX `idx_discussions_created_at` (`created_at` ASC))
