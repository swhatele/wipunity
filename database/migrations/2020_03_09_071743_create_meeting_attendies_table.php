...
class CreateMeetingAttendiesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::unprepared('CREATE TABLE meeting_attendies (
            `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
            `meeting_id` BIGINT NOT NULL,
            `email` VARCHAR(200) NULL,
             `user_id` BIGINT NOT NULL,
            `status` INT NOT NULL,
            `not_attending_reason` INT NULL,
            `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
            `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            PRIMARY KEY (`id`),
            INDEX `idx_meeting_attendies_meeting_id` (`meeting_id` ASC),
            INDEX `idx_meeting_attendies_email` (`email` ASC),
            INDEX `idx_meeting_attendies_status` (`status` ASC),
            INDEX `idx_meeting_attendies_not_attending_reason` (`not_attending_reason` ASC),
            INDEX `idx_meeting_attendies_created_at` (`created_at` ASC))
    ...
    {
        Schema::dropIfExists('meeting_attendies');
    }
}
